...
def make_scaffold_contract_prompt(
    ...
    *,
    ...
    return (
        ...
        "   Preferred benchmark-agnostic implementation: in evaluation.sh call `$AIDER_FSM_PYTHON \"$AIDER_FSM_RUNNER_ROOT/runner/generic_evaluation.py\"`.\n"
        ...
        "7) If you create `.aider_fsm/bootstrap.yml`, keep a strict schema:\n"
        ...
        "   IMPORTANT: `.aider_fsm/bootstrap.yml` is YAML (not a shell script). It MUST NOT start with `#!/bin/bash`.\n"
        "   Minimal valid example (edit for the repo):\n"
        "   version: 1\n"
        "   cmds:\n"
        "     - uv python install 3.11\n"
        "     - uv venv --python 3.11 .aider_fsm/venv\n"
        "     - uv pip install --python .aider_fsm/venv/bin/python -e \".[testing]\"\n"
        "   env:\n"
        ...
        "  - When `AIDER_LLM_KIND=remote`, do NOT use local inference backends (e.g. vLLM/HF/ollama) and do NOT download local models. Use an OpenAI-compatible backend and rely on env-provided model/base URL.\n"
        ...
        "- Built-in helper you may use in stage scripts (runner will set `$AIDER_FSM_PYTHON`, PYTHONPATH, and `AIDER_FSM_RUNNER_ROOT`):\n"
        "  - Generic rollout (writes rollout.json + samples JSONL): `$AIDER_FSM_PYTHON \"$AIDER_FSM_RUNNER_ROOT/runner/generic_rollout.py\"`\n"
        ...
        "- Recommended: `.aider_fsm/bootstrap.yml` for deterministic, non-interactive setup (especially for Python dependency installs).\n"
        ...
        "- If the repo appears to be Python (e.g., has `pyproject.toml` or `requirements*.txt`), create `.aider_fsm/bootstrap.yml`.\n"
        "  It should create an isolated venv under `.aider_fsm/venv`, install deps deterministically, and ensure later stages use the venv.\n"
        ...
        "    - `uv python install 3.11`\n"
        "    - `uv venv --python 3.11 .aider_fsm/venv`\n"
        "    - If `requirements.txt` exists: `uv pip install --python .aider_fsm/venv/bin/python -r requirements.txt`\n"
        "      Else: `uv pip install --python .aider_fsm/venv/bin/python -e .`\n"
        "    - Set env: `PATH: .aider_fsm/venv/bin:$PATH` and `AIDER_FSM_PYTHON: .aider_fsm/venv/bin/python`\n"
