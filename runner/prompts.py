...
def make_scaffold_contract_prompt(repo: Path, *, pipeline_rel: str, require_metrics: bool) -> str:
    ...
    return (
        ...
        "security, artifacts). Do NOT invent a custom schema (e.g. DO NOT write a top-level `stages:` list).\n"
        "\n"
        ...
        "B) Otherwise SAFE no-op: stages may echo `not_configured` and exit 0, but benchmark MUST still write metrics.\n"
        "\n"
        "Hard constraints:\n"
        ...
        "- Benchmark must write a JSON object to `.aider_fsm/metrics.json`.\n"
        "- If benchmark is not configured, write: `{ \"score\": 0, \"note\": \"benchmark_not_configured\" }` (plus optional fields).\n"
        f"{required_line}"
        "\n"
        f"REPO_ROOT: {repo}\n"
